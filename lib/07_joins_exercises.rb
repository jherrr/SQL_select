...
def ford_supporting_films
  ...
  execute(<<-SQL)
    ...
    WHERE
      c.ord != 1 AND a.name = 'Harrison Ford'
  SQL
end
...
def prolific_actors
  ...
  execute(<<-SQL)
    ...
    FROM (
      ...
      GROUP BY
        a.name
    ) actor_movie_num_list
    WHERE
      actor_movie_num_list.num_movies >= 15
    ORDER BY
      name
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    JOIN
      castings cc ON cc.movie_id = mm.id
    JOIN
      actors aa ON cc.actor_id = aa.id
    WHERE
